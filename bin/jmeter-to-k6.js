...
(async () => {
  ...
  if (output) {
    console.log(
      `Starting conversion of: ${chalk.green(path.resolve(input))}\n`
    );
    ...
    console.log(
      `Success! Created test at ${chalk.green(
    ...
    console.log(
      `Please specify the output directory: \n  ${chalk.cyan(
        'jmeter-to-k6'
      )} ${chalk.green('-o <output-dir> <jmx-file>')}\n`
    );
    console.log(
      `For example: \n  ${chalk.cyan('jmeter-to-k6')} ${chalk.green(
        '-o load-test'
      )} ${chalk.green('/path/to/some.jmx')}`
    );
  }
})();
