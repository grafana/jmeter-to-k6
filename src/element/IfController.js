...
function IfController(node, context) {
  ...
  if (settings.condition) {
    const condition = settings.expression
...
`;
    ...
    if (settings.individual) {
      const children = [];
      for (const el of els) {
        children.push(element(el, context));
      }
      const blocks = [];
...
${ind(strip(childrenLogic))}
    ...
    throw new Error('IfController missing condition');
  }
  node.children = [];
  return result;
}
...
function attribute(node, key) {
  switch (key) {
    ...
    case 'testname':
      break;
    default:
      throw new Error(`Unrecognized IfController attribute: ${key}`);
  }
}
...
function property(node, context, settings) {
  ...
  switch (name) {
    ...
    case 'evaluateAll':
      ...
      break;
    default:
      throw new Error(`Unrecognized IfController property: ${name}`);
  }
}
