...
function ResponseAssertion(node, context) {
  ...
  if (!settings.name) {
    settings.name = 'ResponseAssertion';
  }
...
function operand(node, context, _settings) {
  ...
  switch (name) {
    ...
    default:
      throw new Error(`Unrecognized operand type: ${name}`);
  }
}
...
function operation(node, context, settings) {
  ...
  if (bits & 0b1) {
    ...
    throw new Error(`Unrecognized operation spec: ${bits}`);
  }
}
...
function check(settings, result) {
  ...
  for (const test of settings.tests) {
    expressions.push(expr(test, settings));
  }
  const composite = expressions.join(settings.disjunction ? ' || ' : ' && ');
  const logic = `return ${settings.negate ? '!' : ''}(${composite})`;
...
function string(test, settings) {
  if (settings.complete) {
    return completeString(test, settings);
  }
  return partialString(test, settings);
}
...
function completeString(test, settings) {
  ...
  if (/status$/.test(settings.operand)) {
...
function partialString(test, settings) {
  ...
  return `${settings.operand}.includes(${JSON.stringify(test)})`;
}
