...
function RunTime (node, context) {
  ...
  else throw new Error('Invalid RunTime')
  node.children = []
  return result
}
...
function attribute (node, key) {
  switch (key) {
    ...
    case 'testname':
      break
    default: throw new Error('Unrecognized RunTime attribute: ' + key)
  }
}
...
function property (node, context, settings) {
  ...
  switch (name) {
    ...
    case 'seconds':
      ...
      break
    default: throw new Error('Unrecognized RunTime property: ' + name)
  }
}
