...
function property(node, context, settings) {
  ...
  switch (name) {
    ...
    case 'defaultValues':
      ...
      break;
    case 'referenceNames':
      settings.outputs = text(node.children)
        .split(';')
...
function sufficient(settings) {
  if (!(settings.codes && settings.codes.length)) {
    return false;
  }
  const count = settings.codes.length;
  return (
    settings.outputs &&
    settings.outputs.length === count &&
    'index' in settings &&
    (!settings.defaults || settings.defaults.length === count)
  );
}
...
function renderTransport(settings, result) {
  if (settings.index < 0) {
    return renderDistribute(settings, result);
  }
