...
function JSR223PostProcessor(node, context) {
...
function attribute(node, key) {
  switch (key) {
    ...
    case 'testname':
      break;
    default:
      throw new Error(`Unrecognized JSR223PostProcessor attribute: ${key}`);
  }
}
...
function property(node, context, settings) {
  ...
  switch (name) {
    ...
    case 'scriptLanguage':
      ...
      break;
    default:
      throw new Error(`Unrecognized JSR223PostProcessor property: ${name}`);
  }
}
...
function render(settings, result) {
  ...
  if (settings.language) {
    result.logic += `language: ${settings.language}\n\n`;
  }
  if (settings.path) {
...
module.exports = JSR223PostProcessor;
