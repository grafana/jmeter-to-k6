...
function JSONPathExtractor(node, context) {
  ...
  if (sufficient(settings)) {
    ...
    throw new Error('Invalid JSONPathExtractor');
  }
  return result;
}
...
function sufficient(settings) {
  return settings.format && settings.query && settings.input && settings.output;
}
...
function renderInput(settings) {
  ...
  switch (type) {
    case 'BODY':
      return `r.body`;
    case 'VAR':
      if (!settings.inputVar) {
        throw new Error('Missing input var name');
      }
